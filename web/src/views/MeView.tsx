    ...
    <div>
      ...
      <div className="card" style={{ textAlign: 'center', marginBottom: 'var(--space-md)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-sm)', marginTop: 'var(--space-sm)' }}>
          ...
          <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 600, color: 'var(--color-foreground)' }}>
            ผู้เล่นเลเวล {level}
          </span>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 'var(--space-md)' }}>
        <h3 className="section-title" style={{ marginTop: 0 }}>📜 ประวัติการเล่น</h3>
        {data.history.length === 0 ? (
          ...
          data.history.map((h: any, i: number) => (
            <div key={i} className="history-item">
              ...
              <span style={{ color: 'var(--color-muted-foreground)' }}>{h.game_id === 'quiz' ? 'คำถาม' : 'จับคู่ข้าวโพด'}</span>
      ...
      {/* Coupons */}
      <div className="card">
        <h3 className="section-title" style={{ marginTop: 0 }}>🎫 คูปองที่แลก</h3>
